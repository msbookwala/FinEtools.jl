...
- `afield` = NODAL field to supply the values at the nodes, which are
  interpolated to the quadrature points,
- `fh` = function taking position and an array of field values for the element
  as arguments, returning value of type `R`.  The function `fh` must take two
  arguments, `x` which is the location, and `val` which is the value of the
  field at that location. The rectangular array of field values `val`  has one
  row, and as many columns as there are degrees of freedom per node.
...
Integrates a  function returning a scalar value of type `R`, which is
...
- `afield` = ELEMENTAL field to supply the value within the element (one value
  per element),
- `fh` = function taking position and an array of field values for the element
  as arguments, returning value of type `R`.  The function `fh` must take two
  arguments, `x` which is the location, and `val` which is the value of the
...
function bilform_convection(
    ...
    npts, Ns, gradNparams, w, pc = integrationdata(self.integdomain)
    startassembly!(assembler, size(elmat)..., count(fes), nalldofs(Q), nalldofs(Q))
    for i in eachindex(fes) # Loop over elements
        gathervalues_asmat!(geom, ecoords, fes.conn[i])
